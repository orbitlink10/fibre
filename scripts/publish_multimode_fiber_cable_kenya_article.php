...
$article['id'] = empty($pages) ? 1 : (max(array_map(fn ($page) => (int) ($page['id'] ?? 0), $pages)) + 1);
...
preg_match_all('/<img\s+[^>]*src=/i', $article['description'], $images);

echo $article['slug'].PHP_EOL;
echo 'words: '.str_word_count($plain).PHP_EOL;
echo 'links: '.count($links[0]).PHP_EOL;
echo 'inline_images: '.count($images[0]).PHP_EOL;
echo 'visible_images_including_hero: '.(count($images[0]) + 1).PHP_EOL;
