...
$heroPath = realpath(__DIR__.'/../storage/app/public/pages/fiber-optic-network-installation-kenya-hero.jpg');
...
if (! $article) {
    fwrite(STDERR, "Article not found in pages.json. Run publish_fiber_optic_network_installation_kenya_article.php first.\n");
    exit(1);
}
...
[$status, $headers, $body] = request($baseUrl.'/pages', [
    ...
    CURLOPT_POSTFIELDS => [
        ...
        'image' => new CURLFile($heroPath, 'image/jpeg', 'fiber-optic-network-installation-kenya-hero.jpg'),
    ],
]);

if ($status !== 302) {
    file_put_contents(__DIR__.'/../storage/logs/fiber_network_installation_remote_publish_failed.html', $body);
    throw new RuntimeException("Post submission returned HTTP {$status}. Response saved to storage/logs/fiber_network_installation_remote_publish_failed.html");
}
