...
require __DIR__.'/multimode_fiber_cable_kenya_article.php';

$baseUrl = 'https://fiberopticskenya.co.ke';
$email = 'admin@demo.com';
$password = 'admin123';
$article = multimode_fiber_cable_kenya_article();
...
function findPageId(string $html, string $title): ?int
{
    ...
    foreach ($rows[0] as $row) {
        if (str_contains(html_entity_decode(strip_tags($row), ENT_QUOTES), $title) && preg_match('/\/pages\/(\d+)\/edit/', $row, $match)) {
            return (int) $match[1];
        }
    }
    return null;
...
[$status, , $formHtml] = request($isUpdate ? $baseUrl.'/pages/'.$pageId.'/edit' : $baseUrl.'/new-post');
if ($status !== 200) {
...
$fields = [
    ...
    'image' => new CURLFile($heroPath, 'image/png', 'multimode-fiber-cable-kenya-hero.png'),
];
if ($isUpdate) {
    $fields['_method'] = 'PUT';
}

[$status, , $body] = request($isUpdate ? $baseUrl.'/pages/'.$pageId : $baseUrl.'/pages', [
    CURLOPT_POST => true,
    CURLOPT_POSTFIELDS => $fields,
]);
if ($status !== 302) {
    file_put_contents(__DIR__.'/../storage/logs/multimode_fiber_remote_publish_failed.html', $body);
    throw new RuntimeException("Post submission returned HTTP {$status}. Response saved to storage/logs/multimode_fiber_remote_publish_failed.html");
}
