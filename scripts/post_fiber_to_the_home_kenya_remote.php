...
function findPageId(string $html, string $title): ?int
{
    ...
    foreach ($rows[0] as $row) {
        if (str_contains(html_entity_decode(strip_tags($row), ENT_QUOTES), $title)) {
...
$fields = [
    ...
    'image' => new CURLFile($heroPath, 'image/png', 'fiber-to-the-home-kenya-hero.png'),
];
...
[$status, , $body] = request($postUrl, [
    CURLOPT_POST => true,
    CURLOPT_POSTFIELDS => $fields,
]);

if ($status !== 302) {
    file_put_contents(__DIR__.'/../storage/logs/fiber_to_the_home_remote_publish_failed.html', $body);
    throw new RuntimeException("Post submission returned HTTP {$status}. Response saved to storage/logs/fiber_to_the_home_remote_publish_failed.html");
}
